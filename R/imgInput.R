imgInput <-
function(img, dpi = NULL, rotate = 0, RGB = c(0.299, 0.587, 0.114), magick = TRUE)
{
    
    check.degree <- rotate %in% c(0, 90, 180, 270, 360) 
    if (!check.degree)
        stop('The argument \'rotate\' should be one of ',
    ...
    if (names(dev.cur()) == "RStudioGD") 
        dev.new()
    grD.num <- as.numeric(dev.cur())

    if (is.character(img)) {  
      img.name <- basename(img)
    } else {
      img.name <- substitute(img) %>% as.character   
    }
    
    
    attributes(tdata) <- c(attributes(tdata), 
                         list(x.dpi = dpi, grD.num = grD.num,
                              RGB = RGB,  img.name = img.name,
                              dimt = dim.tdata))
    ...
    plot(x = 0, y = 0, main = attributes(tdata)$img.name, 
       xlim = c(xleft, xright), ylim = c(ybottom, ytop), 
       xlab = '', ylab = '', type = 'n', axes = F, cex.main = 1.2)
    axis(1, col = "grey", cex.axis = 1)
    axis(2, col = "grey", cex.axis = 1)
    rasterImage(as.raster(tdata.copy), xleft, ybottom, xright, ytop, interpolate = FALSE)
